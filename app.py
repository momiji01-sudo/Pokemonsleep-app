...
st.markdown("""
    ...
    [data-testid="stHorizontalBlock"] > div {
        flex: 1 1 45% !important; /* 50%弱に固定 */
        min-width: 45% !important;
    }
    
    /* 食材配列用の3列強制 (クラス名等で制御できないため、n-childで調整) */
    .ing-container [data-testid="stHorizontalBlock"] > div {
    ...
    .stCheckbox label p { font-size: 0.7rem !important; white-space: nowrap !important; }
...
ALL_SKILLS = [
    # 金スキル系統
    "🟡きのみの数S", "🟡おてつだいボーナス", "🟡スキルレベルアップM", "🟡睡眠EXPボーナス", 
    "🟡げんき回復ボーナス", "🟡ゆめのかけらボーナス", "🟡リサーチEXPボーナス",
    # 青・白スキル系統 (スピ・食材・スキル・所持の順)
    "🔵おてつだいスピードM", "⚪おてつだいスピードS",
    "🔵食材確率アップM", "⚪食材確率アップS",
    "🔵スキル確率アップM", "⚪スキル確率アップS",
    "🔵スキルレベルアップS",
    "🔵最大所持数アップL", "🔵最大所持数アップM", "⚪最大所持数アップS"
]
...
def get_theoretical_probability(allow_imprison):
    ...
    match_count = 0; total_patterns = 0
    def solve(depth, current_skills):
            ...
            if not allow_imprison:
                for upper, lower in SUB_PAIRS:
                    if lower in current_skills and upper in current_skills:
                        if current_skills.index(lower) < current_skills.index(upper): return
            total_patterns += 1
        ...
        for s in available:
            current_skills.append(s); solve(depth + 1, current_skills); current_skills.pop()

    if not any(filters) and not must_have and allow_imprison: p_sub = 1.0
    else: solve(0, []); p_sub = match_count / total_patterns if total_patterns > 0 else 0
...
for g_label, natures in NATURE_GROUPS.items():
    ...
    # 見出しとボタンの行 (4カラム指定だが実質2:1:1:余白)
    h_cols = st.columns([2, 1, 1, 0.1])
    ...
    for j in range(0, len(natures), 2):
        ...
        with r_cols[0]:
            n1, s1 = natures[j]
            st.checkbox(f"{n1}({s1})", key=f"n_{n1}")
        if j + 1 < len(natures):
            with r_cols[1]:
                n2, s2 = natures[j+1]
